		   ...
		   int showscale, int lowcolor, int hatch, int plotcontour)
{
	...
	float xz,xv12,yv12,xv23,yv23,xv34,yv34,xv41,yv41;
	float cx,cy,cbx,cby,cbbox,polytolx,polytoly;
	float *xv,*yv,*red,*blue,*green,*dd,*xvv,*yvv,*xspoly,*yspoly,*xrpoly,*yrpoly;
	float **wsp;

	cx = 50; //origin of contour plot
	cy = 100;//origin of contour plot
	cbbox = 15.; //size of boxes
	cbx = 570; //origin of color bar
	cby = 100;//origin of color bar
	polytolx = 1.e-4*(xmax - xmin)/(m-1);
	polytoly = 1.e-4*(ymax - ymin)/(n-1);
	...
	xspoly = vector(1,6);
	yspoly = vector(1,6);
	xrpoly = vector(1,nrpolym);
	yrpoly = vector(1,nrpolym);
	dd = vector(1,4);
	ii = ivector(1,4);
	jj = ivector(1,4);
	corners[1][1] = 0;
...
//Set up colors using Matlab 'jet' scheme
	for(k=0; k<=nl; k++){
		xz = float(k)/float(nl);
		blue[k] = fmin(fmax(1.5-4*abs(xz-0.25), 0.), 1.);
		green[k]= fmin(fmax(1.5-4*abs(xz-0.5), 0.), 1.);
		red[k]  = fmin(fmax(1.5-4*abs(xz-0.75), 0.), 1.);
	}
...
//combine s-polygon with existing r-polygon, eliminating redundant segments
								if(nrpoly == 0){						//initiate r-polygon
									...
									for(irpoly=nrpoly; irpoly>=1; irpoly--) for(ispoly=1; ispoly<=nspoly; ispoly++){
										...
										if((abs(xrpoly[irpoly] - xspoly[ispoly1]) < polytolx)
											&& (abs(yrpoly[irpoly] - yspoly[ispoly1]) < polytoly)
											&& (abs(xrpoly[irpoly1] - xspoly[ispoly]) < polytolx)
											&& (abs(yrpoly[irpoly1] - yspoly[ispoly]) < polytoly)) goto foundit3;
									}
									...
									for(iipoly=2; iipoly<nspoly; iipoly++){	//search for further matches nearby 
										...
										if((abs(xrpoly[irpoly1] - xspoly[ispoly1]) < polytolx)
											&& (abs(yrpoly[irpoly1] - yspoly[ispoly1]) < polytoly)) match1++;
										else goto nomatch1;
									}
									nomatch1:;
									match2 = 0;
									for(iipoly=2; iipoly<nspoly; iipoly++){	//search other way for further matches nearby 
										...
										if((abs(xrpoly[irpoly1] - xspoly[ispoly1]) < polytolx)
											&& (abs(yrpoly[irpoly1] - yspoly[ispoly1]) < polytoly)) match2++;
										else goto nomatch2;
									}
									nomatch2:;
									...
									if((abs(xrpoly[irpolystart] - xspoly[ispolyend]) > polytolx)
										|| (abs(yrpoly[irpolystart] - yspoly[ispolyend]) > polytoly))
										printf("*** Error: r and s nodes do not match ****\n");
									if((abs(xrpoly[irpolyend] - xspoly[ispolystart]) > polytolx)
										|| (abs(yrpoly[irpolyend] - yspoly[ispolystart]) > polytoly))
										printf("*** Error: r and s nodes do not match ****\n");
									...
									if(nrpoly < irpolyend) for(iipoly=irpolyend; iipoly>nrpoly; iipoly--){	//otherwise these vaues get lost!
...
//display polygons after combining
				...
				if(hatch > 0){
					...
					fprintf(ofp, "diagonals grestore\n");
				}
				else fprintf(ofp, "cf\n");
			}
		}
...
//create a color bar
	...
	free_vector(xvv,1,4);
	free_vector(yvv,1,4);
	free_vector(red,0,nl);
	free_vector(green,0,nl);
	free_vector(blue,0,nl);
	free_matrix(wsp,1,ncmax,1,4);
	free_imatrix(corners,1,4,1,2);
	free_imatrix(sqtype,1,m-1,1,n-1);
	free_vector(xspoly,1,6);
	free_vector(yspoly,1,6);
	free_vector(xrpoly,1,nrpolym);
	free_vector(yrpoly,1,nrpolym);
	free_vector(dd,1,4);
	free_ivector(ii,1,4);
	free_ivector(jj,1,4);
}
